...
# png("graphics/pv2p_2020_plot.png", units="in", width=7, height=5, res=300)
# print(sim_2020_plot)
# dev.off()
...
state_2020_pred_data <- state_data_2020 %>% 
  ...
  mutate(pred = map_dbl(.x = data, .y = model, 
                        ~predict(.y, newdata = .x, type = "response")[[1]]),
         vep = map_dbl(data, ~.x$VEP)) %>% 
  select(-c(data, model))
...
# Creaing simulated draws

# mutate(pred_dist = map(.x = pred, .y = vep, ~rbinom(n = 10000, size = .y, prob = .x)))
